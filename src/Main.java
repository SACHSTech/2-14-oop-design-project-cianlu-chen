            ...
            switch(choice){
                    ...
                    for(int i = 0; i < store.getNumCarts(); i++){
                        if(store.getShoppingCart(i).getUserID() == choice){
                            for(int j = 0; j < store.getShoppingCart(choice).getNumItems(); j++){
                                System.out.println(store.getShoppingCart(choice).getProduct(j).getName());
                            }
                        }
                    }
                ...
                case 7:
                    ...
                    for(int i = 0; i < store.getNumCarts(); i++){
                        if(store.getShoppingCart(i).getUserID() == choice){
                            for(int j = 0; j < store.getShoppingCart(choice).getNumItems(); j++){
                                System.out.println(store.getShoppingCart(choice).getProduct(j).getName() + " ID: " + store.getShoppingCart(choice).getProduct(j).getID());
                            }
                        }
                    }
                ...
                case 8:
                    ...
                    if(store.getShoppingCart(choice).isEmpty()){
                        ...
                        for(int i = 0; i < store.getShoppingCart(choice).getNumItems(); i++){
                            System.out.println(store.getShoppingCart(choice).getProduct(i).getName() + ", $" + store.getShoppingCart(choice).getProduct(i).getPrice());
                        }
